...
mod tests {
    ...
    #[test]
    fn test_parse_terminal_with_tmux() {
        let parsed = parse_window_name(
            "gnome-terminal",
            "tmux: editor - adolfo@adolfo-ubuntu-pro25: ~/projects/myapp"
        );

        assert_eq!(parsed.terminal_username, Some("adolfo".to_string()));
        assert_eq!(parsed.terminal_hostname, Some("adolfo-ubuntu-pro25".to_string()));
        assert_eq!(parsed.tmux_window_name, Some("editor".to_string()));
        assert_eq!(parsed.terminal_multiplexer, Some("tmux".to_string()));
        // Directory should be expanded from ~ to actual home path
        assert!(parsed.terminal_directory.as_ref().unwrap().starts_with("/home/"));
        assert!(parsed.terminal_directory.as_ref().unwrap().ends_with("/projects/myapp"));
        assert_eq!(parsed.terminal_project_name, Some("myapp".to_string()));
    }

    #[test]
    fn test_parse_terminal_home_directory() {
        let parsed = parse_window_name(
            "gnome-terminal",
            "user@host: ~"
        );

        assert_eq!(parsed.terminal_username, Some("user".to_string()));
        assert_eq!(parsed.terminal_hostname, Some("host".to_string()));
        // Directory should be expanded to home
        assert!(parsed.terminal_directory.as_ref().unwrap().starts_with("/home/"));
        assert_eq!(parsed.terminal_project_name, Some("Home".to_string()));
    }

    #[test]
    fn test_parse_terminal_home_subdirectory() {
        let parsed = parse_window_name(
            "gnome-terminal",
            "user@host: ~/Documents"
        );
        ...
        assert_eq!(parsed.terminal_username, Some("user".to_string()));
        assert_eq!(parsed.terminal_hostname, Some("host".to_string()));
        // Directory should be expanded to home/Documents
        assert!(parsed.terminal_directory.as_ref().unwrap().starts_with("/home/"));
        assert!(parsed.terminal_directory.as_ref().unwrap().ends_with("/Documents"));
        assert_eq!(parsed.terminal_project_name, Some("Documents".to_string()));
    }

    #[test]
    fn test_extract_tmux_info() {
        // Test tmux: pattern - focus on tmux info extraction, not exact string format
        let (_, info) = extract_tmux_info("tmux: editor - user@host: ~/project");
        assert_eq!(info, Some(("editor".to_string(), None)));
        ...
        // Test - tmux (pattern)
        let (_, info) = extract_tmux_info("user@host: ~/project - tmux (editor)");
        assert_eq!(info, Some(("editor".to_string(), None)));

        // Test no tmux
        let (_, info) = extract_tmux_info("user@host: ~/project");
        assert_eq!(info, None);
    }
    ...
    #[test]
    fn test_extract_project_name_improved() {
        // Test with actual HOME environment variable if available
        if let Ok(home) = std::env::var("HOME") {
            assert_eq!(extract_project_name(&home), Some("Home".to_string()));
            assert_eq!(extract_project_name(&format!("{}/Documents", home)), Some("Documents".to_string()));
            assert_eq!(extract_project_name(&format!("{}/projects/myapp", home)), Some("myapp".to_string()));
        }

        assert_eq!(extract_project_name("/srv/rs/project"), Some("project".to_string()));
        assert_eq!(extract_project_name("/usr/bin"), None); // Should skip common system dirs
        assert_eq!(extract_project_name("/home/user"), Some("user".to_string()));
    }
}
